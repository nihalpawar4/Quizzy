    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            snapshot.docs.forEach((doc) => {
                const data = doc.data();
                if (data.isActive && !data.isExclusiveTest) {
                    allTests.push({
                        id: doc.id,
                        ...data,
            ...
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    ...
                    if (timeDiff < 30000 && data.isActive && !data.isExclusiveTest) {
                        const newTest: Test = {
                            id: change.doc.id,
                            ...data,
                            createdAt,
...
function PremiumFeaturesTab({ currentStreak, longestStreak, lastStreakDate, isPremium, user: tabUser }: PremiumFeaturesTabProps) {
    ...
    // ── Exclusive Test State ─────────────────────────────────────────
    const [exclusiveTickets, setExclusiveTickets] = useState(0);
    const [exclusiveTests, setExclusiveTests] = useState<Test[]>([]);
    const [exclusiveLoading, setExclusiveLoading] = useState(false);
    const [exclusiveSubject, setExclusiveSubject] = useState('All');
    const [activeExclusiveTest, setActiveExclusiveTest] = useState<Test | null>(null);
    ...
    useEffect(() => {
        ...
        async function loadExclusiveData() {
            ...
            try {
                ...
                await Promise.all(
                    tests.map(async (t) => {
                        const done = await hasCompletedExclusiveTest(tabUser!.uid, t.id);
                        if (done) completedSet.add(t.id);
                    })
                );
                setCompletedExclusiveIds(completedSet);
            } catch (err) {
                console.error('[Exclusive] Load error:', err);
            }
            setExclusiveLoading(false);
        }

        loadExclusiveData();
    }, [isPremium, tabUser?.uid, tabUser?.studentClass, exclusiveSubject]);

    // Confirm spend tickets & start exclusive test
    const handleExclusiveConfirm = useCallback(async () => {
        if (!exclusiveConfirm || !tabUser?.uid || ticketSpending) return;
        setTicketSpending(true);
        try {
            const result = await spendTickets(tabUser.uid, EXCLUSIVE_TICKETS_PER_TEST);
            if (!result.success) {
                alert(result.error || 'Not enough tickets.');
                setTicketSpending(false);
                return;
            }
            setExclusiveTickets(prev => Math.max(0, prev - EXCLUSIVE_TICKETS_PER_TEST));
            setActiveExclusiveTest(exclusiveConfirm);
            setExclusiveConfirm(null);
        } catch (err) {
            console.error('[Exclusive] Ticket spend error:', err);
        }
        setTicketSpending(false);
    }, [exclusiveConfirm, tabUser?.uid, ticketSpending]);

    const handleExclusiveComplete = useCallback(() => {
        if (activeExclusiveTest) {
            setCompletedExclusiveIds(prev => new Set([...prev, activeExclusiveTest.id]));
        }
    ...
    if (activeExclusiveTest && tabUser) {
        return (
            <ExclusiveTestScreen
                test={activeExclusiveTest}
                user={tabUser}
                onComplete={handleExclusiveComplete}
                onClose={() => setActiveExclusiveTest(null)}
            />
        );
    }
                        ...
                        >
                            ...
                            <div className="bg-gradient-to-r from-amber-500 via-yellow-500 to-orange-500 px-6 py-5 text-center text-white relative overflow-hidden">
                                ...
                                <div className="relative">
                                    ...
                                    <h3 className="font-bold text-lg">Spend Tickets?</h3>
                                    <p className="text-white/70 text-sm mt-1">{exclusiveConfirm.title}</p>
                                </div>
                            </div>
                            ...
                            <div className="p-6 text-center">
                                <div className="flex items-center justify-center gap-4 mb-4">
                                    <div className="text-center">
                                        <p className="text-2xl font-extrabold text-amber-500">{EXCLUSIVE_TICKETS_PER_TEST}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-gray-500">Tickets used</p>
                                    </div>
                                    <div className="text-gray-300 dark:text-gray-600">→</div>
                                    <div className="text-center">
                                        <p className="text-2xl font-extrabold text-amber-500">{exclusiveTickets - EXCLUSIVE_TICKETS_PER_TEST}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-gray-500">Remaining</p>
                                    </div>
                                </div>

                                <div className="flex items-center justify-center gap-2 mb-5 text-[11px] font-medium px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-500/8 text-amber-600 dark:text-amber-400">
                                    <Zap className="w-3.5 h-3.5" />
                                    You&apos;ll earn +{EXCLUSIVE_TEST_XP_REWARD} XP on completion
                                </div>
